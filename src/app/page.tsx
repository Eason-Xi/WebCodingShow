    ...
    <div className="shell w-full py-10">
      ...
      <section className="grid grid-cols-1 gap-10 lg:grid-cols-[1.12fr_0.88fr] lg:items-center">
        <div className="animate-rise">
          <Chip tone="lime" dot className="font-mono tracking-wide text-xs sm:text-[13px] px-3 py-1">
            00 · AI 访谈第二导演 · 全流程对谈工作坊
          </Chip>
          <h1 className="mt-5 text-[38px] font-extrabold leading-[1.16] tracking-tight text-1 sm:text-[48px]">
            提问交给 AI，<br className="hidden sm:inline" />
            <span className="text-lime-600 dark:text-lime-400">深度</span>留给对谈。
          </h1>
          <p className="mt-4 max-w-xl text-base leading-relaxed text-2">
            覆盖「全网画像研究 → 分级策划大纲 → 高保真 AI 彩排对练 → 逐字稿提炼 → 爆款短视频拆条」全生命周期，
            让 AI 成为您身旁不知疲倦的第二导演。
          </p>
          <div className="mt-8 flex flex-wrap items-center gap-3.5">
            <button onClick={() => setShowModal(true)} className="btn btn-primary btn-lg text-base">
              <Plus className="h-4 w-4 stroke-[2.5]" />
              新建访谈项目
            </button>
            <a href="#projects-list" className="btn btn-secondary btn-lg text-base">
              浏览项目库 ({projects.length})
              <ArrowRight className="h-4 w-4" />
            </a>
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-3 font-mono text-xs text-3">
            <span className="inline-flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-lime-500 dark:bg-lime-400" />
              全流程上下文已激活
            </span>
            <span>·</span>
        ...
        <div className="hero-console animate-rise stagger-2">
          <div className="console-top">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 font-mono text-xs uppercase tracking-wider text-slate-200">
                AI DIRECTOR KERNEL
              </span>
            </div>
            <div className="flex items-center gap-2 font-mono text-xs">
              ...
              <span className="text-slate-200">118ms</span>
            </div>
          </div>

          <div className="space-y-2 p-5 font-mono text-xs sm:text-[13px] leading-relaxed console-val">
          ...
          <div className="p-4 pt-0">
            <div className="console-result flex items-center justify-between text-xs sm:text-sm">
              ...
              <span className="font-mono text-xs font-semibold console-lime">confidence .94</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mt-12 stat-strip grid grid-cols-2 divide-y divide-line-1 sm:grid-cols-4 sm:divide-x sm:divide-y-0 p-3 sm:p-5">
        {STATS.map((s) => (
          ...
          >
            <span className="text-xs font-semibold text-3 font-mono">{s.label}</span>
            <div className="mt-1.5 flex items-baseline gap-1.5">
              <span className="text-[34px] font-extrabold tracking-tight tabular text-1">
                {s.value}
              </span>
              <span className="text-sm text-2 font-mono">{s.unit}</span>
            </div>
            <p className="mt-1 text-xs text-3">{s.hint}</p>
          </div>
        ))}
      </section>
      ...
      <section id="projects-list" className="mt-14 scroll-mt-24">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="eyebrow text-lime-600 dark:text-lime-400 font-mono mb-1.5 text-xs sm:text-[13px]">
              01 · 访谈项目档案库 (PROJECT ARCHIVES)
            </div>
            <h2 className="flex items-center gap-2.5 text-2xl font-bold tracking-tight text-1">
              我的访谈项目
            </h2>
            <p className="mt-1.5 text-sm text-2">点击卡片进入深度策划、模拟对谈与视频包装工作台</p>
          </div>
            ...
            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
              <div className="relative">
                ...
                <input
                  ...
                  aria-label="搜索项目"
                  className="field h-10 w-full pl-9 pr-8 text-sm sm:w-64"
                />
                {query && (
                  <button
                    onClick={() => setQuery("")}
                    aria-label="清除搜索"
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded p-1 text-3 transition-colors hover:text-1"
                  >
                    <X className="h-4 w-4" />
                  </button>
                )}
              ...
              <select
                ...
                aria-label="排序方式"
                className="field h-10 w-full text-sm sm:w-[130px]"
              >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            {[
              ...
              <button
                ...
                className={clsx(
                  "btn btn-sm rounded-full border text-xs sm:text-[13px] px-3.5 py-1.5 transition-all duration-200",
                  statusFilter === f.key
                    ? "bg-lime-400 text-[#0b0c0f] font-bold border-transparent shadow-[0_2px_12px_rgba(201,255,99,0.3)]"
                    : "border-line-1 bg-surface-1 text-2 hover:bg-surface-2 hover:text-1"
                )}
              >
                {f.label}
                <span className={clsx("tabular text-xs font-semibold", statusFilter === f.key ? "text-[#0b0c0f]/80" : "text-3")}>
            ...
            {hasActiveFilter && (
              <button
                ...
                className="btn btn-ghost btn-sm ml-1 text-xs sm:text-[13px] text-3 hover:text-1"
              >
            ...
            <span className="tabular ml-auto text-xs text-3 font-mono">
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm font-bold text-3 group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors">
                        {serial}
                      </span>
                      <Chip tone="lime" className="text-xs py-0.5 px-2.5">
                  ...
                  <div className="relative mt-4 flex items-center gap-3.5">
                    <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border border-line-2 bg-surface-2 text-base font-bold text-1 transition-transform group-hover:scale-105">
                      {project.guestName?.slice(0, 1) || "?"}
                    </span>
                    <div className="min-w-0">
                      <h3 className="truncate text-lg font-bold tracking-tight text-1 transition-colors group-hover:text-lime-600 dark:group-hover:text-lime-300">
                        {project.guestName}
                      </h3>
                      <p className="truncate text-xs sm:text-[13px] text-3">
                  ...
                  {/* 主题引述 */}
                  <div className="inset relative mt-4 line-clamp-2 rounded-xl p-3.5 text-sm leading-relaxed text-1 border border-line-1">
                    {project.topic}
                  </div>
                  ...
                  <div className="relative mt-4">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <Chip tone={status.tone} dot className="px-2.5 py-0.5 text-xs">
                        {status.label}
                      </Chip>
                  ...
                  {/* 底部指标 */}
                  <div className="relative mt-4 flex items-center justify-between border-t border-line-1 pt-3.5">
                    <div className="flex items-center gap-3.5 text-xs sm:text-[13px] text-2 font-mono">
                      <span className="flex items-center gap-1.5">
                        <FileText className="h-4 w-4 text-3" />
                        <span className="tabular text-1 font-bold">{metrics.questions}</span> 题
                      </span>
                      <span className="flex items-center gap-1.5">
                        <Video className="h-4 w-4 text-3" />
                        <span className="tabular text-1 font-bold">{metrics.videos}</span> 拆条
                      </span>
                    </div>
                    <span className="flex items-center gap-1 text-xs sm:text-sm font-bold text-lime-600 dark:text-lime-400 group-hover:translate-x-0.5 transition-transform">
                      进入工作台
                      <ArrowRight className="h-4 w-4" />
                    </span>
                  </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2
                    ...
                    className="text-lg font-bold tracking-tight text-1"
                  >
                    创建新访谈项目
                  </h2>
                  <p className="mt-1.5 text-sm leading-relaxed text-2">
